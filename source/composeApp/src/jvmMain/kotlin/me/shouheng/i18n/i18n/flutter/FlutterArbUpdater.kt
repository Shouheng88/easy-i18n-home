...
/** Flutter arb 单词更新逻辑 */
object FlutterArbUpdater: IWordUpdater {
    override fun update(
        word: I18nWordModel,
        ...
        items.filter { it.changed }.forEach {
            ...
            val resource = FlutterArbResource(word.name, it.value, null, meaning.file)
            val succeed = FlutterArbParser.update(resource, word.order, meaning.file)
            if (!succeed) {
                loge { "更新词条失败！" }
